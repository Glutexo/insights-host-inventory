...
class KafkaEventProducer:
    ...
    def write_event(self, event, host_id):
        logger.debug(f"Topic: {self._topic}, Key: {host_id} => {event}")

        try:
            self._kafka_producer.send(self._topic, key=host_id.encode("utf-8"), value=event.encode("utf-8"))
